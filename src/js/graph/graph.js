...
class App {
  ...
  startGeneratingLines() {
    ...
    function generateItem() {
      ...
      item.length = tools.random(xCell * 2, state.area * xCell * 6);
  ...
  startGeneratingGlitches() { return null }
  drawGlitches() {
    const ctx = context.main;
    const state = this.state;
  ...
  // animation of graph grids and axes --------------------------------------
  drawPlane() {
    const state = this.state;
    const ctx = context.plane;
  ...
  // rest --------------------------------------------------
  drawMarkupYAnimation(props) { return null }
  drawMarkupXAnimation(props) { return null }
  drawMouseMoveInteraction(props) { return(null) }
